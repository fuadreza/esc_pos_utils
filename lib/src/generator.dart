...
class Generator {
  ...
  Uint8List _encode(String text, {bool isKanji = false}) {
    ...
    text = text.replaceAll("’", "'").replaceAll("´", "'").replaceAll("»", '"').replaceAll(" ", ' ').replaceAll("•", '.').replaceAll('–', '-').replaceNonPrintable(replaceWith: '?');
    if (!isKanji) {
  ...
  /// with option to give extra new lines with [newLines]
  List<int> cut({PosCutMode mode = PosCutMode.full, int newLines = 0}) {
    List<int> bytes = [];
    bytes += emptyLines(newLines);
